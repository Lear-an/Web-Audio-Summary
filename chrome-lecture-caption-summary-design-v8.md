...
확장 프로그램은 `/health/ready`로 서버를 깨운 뒤 세션을 생성합니다. 세션 생성은 정규화한 영상 메타데이터를 받고 `session_id`, `document_id`를 반환합니다.
...
패널 시작 시 `/health/ready`로 서버를 깨우고 연결 상태를 확인합니다. 세션 생성과 tabCapture 권한 확보 후에만 캡처를 시작합니다. 활성 스트림이 이미 있으면 중복 캡처 대신 기존 세션 복귀를 안내합니다.
