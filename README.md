...
- 전사에 성공한 자막은 즉시 확정되며, 다음 청크와 겹치는 경계 문장은 추가 API 호출 없이 로컬에서 중복 제거하거나 부분 병합합니다.
- 영상 탐색·일시정지·배속 변경 시 현재 창을 끊어 영상 타임스탬프를 다시 맞춥니다.
- 대기 오디오 큐가 24MB를 넘으면 경고하고, 32MB에 도달하면 캡처를 일시정지하며, 12MB 이하에서 재개합니다.
- Gemini가 429 할당량 소진 응답을 반환하면 실패 청크와 큐를 보존한 채 캡처를 일시정지합니다. 사이드패널에 새 API 키를 입력하고 `새 키로 계속`을 누르면 같은 세션을 재개합니다.
- Gemini가 503 모델 혼잡 응답을 반환하면 오디오 청크를 메모리 큐에 보존하고 15·30·45초 간격으로 다시 시도합니다.
- 중간 요약은 성공적으로 처리된 전사 청크 3개마다 한 번 실행합니다. 300초 청크 설정에서는 약 15분마다 실행됩니다.
- 캡처 종료 시 남은 청크 처리를 5~15분 기다리며, 제한시간 뒤 누락된 청크가 있으면 개수를 명확히 표시합니다. 종료 처리 중에는 브라우저와 서버를 닫지 마세요.
